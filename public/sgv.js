...
function SocialGraphVisualizer(options = {}) {
  ...
  this.svg = select(options.selector);
  if (!this.svg) {
...
SocialGraphVisualizer.prototype.render = function () {
  ...
  var simulation = simulation().force('link', link().id(d => d.id)).force('charge', manyBody()).force('center', center$1(this.svg.attr('width') / 2, this.svg.attr('height') / 2));

  var link = this.svg.append('g').attr('class', 'links').selectAll('line').data(this.graph.links).enter().append('line').attr('stroke-width', d => Math.sqrt(d.value));

  var node = this.svg.append('g').attr('class', 'nodes').selectAll('circle').data(this.graph.nodes).enter().append('circle').attr('r', 5).attr('fill', d => color$$1(d.group)).call(drag().on('start', dragstarted).on('drag', dragged).on('end', dragended));
